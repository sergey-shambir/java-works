...
public class PriceCalculator {
    ...
    public final BigDecimal getProductsCost(BigDecimal discountPercentage,
                                            Product product, int productCount) {
        assert productCount > 0;

        // In holy C++, you can write this way:
        //   product.getPrice() * productCount * ((fullPercentage -
        //   discountPercentage) / fullPercentage);
        // In damned Java, you will write another way:

        final BigDecimal pricePercentage =
            fullPercentage.subtract(discountPercentage);
    ...
    public final BigDecimal getProductBonuses(Product product,
                                              BigDecimal cost) {
        assert(cost.compareTo(BigDecimal.ZERO) > 0);
        return cost.multiply(product.getBonusesPercentage())
            .divide(fullPercentage);
    }

    public final BigDecimal getCustomerBasketCost(Customer customer) {
        final BigDecimal discountPercentage =
            new BigDecimal(this.discountInfo.getDiscountPercentage(customer));
    ...
    public final BigDecimal getBasketCost(Basket basket,
        ...
        for (HashMap.Entry<Product, Integer> entry : products.entrySet()) {
            ...
            price = price.add(
                getProductsCost(discountPercentage, product, productCount));
        }
        return price;
    }

    public BigDecimal getBasketBonuses(Basket basket,
                                       BigDecimal discountPercentage) {
